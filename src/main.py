...
def main():
    ...
    print(
        f"ARIMA Model - MAE: {model_results['arima_mae']:.4f}, RMSE: {model_results['arima_rmse']:.4f}\n"
    )
    ...
    if arima_model is not None:
        ...
        # Use simple forecast if ARIMA failed
        future_forecast = np.full(252, returns_data['TSLA'].mean())
        print(f"Generated baseline forecast of {len(future_forecast)} days\n")
    ...
    max_sharpe_return, max_sharpe_vol = optimizer.portfolio_performance(
        max_sharpe_weights
    )
    ...
    results_path = Config.get_results_path('portfolio_results.json')
    pd.DataFrame([results]).to_json(results_path, indent=2)
